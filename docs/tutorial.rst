...
For the purpose of the tutorial however, we will attempt to prepare the whole dataset for use. 
First, when reading a file like this one, all input data is in string(bytes) format. For slices this is not the ideal format.
Therefore, we cast the necessary fields from bytes to int and real::
...
However, both fields use different approaches to uppercase/lowercase, so before matching we will change both to use always upper case.
    ...
    >>> tftargetdat = tftargetdata.To(_.feat_name, Do=_.Each(str.upper))
    >>> yeastract = yeastract.To(_.target,         Do=_.Each(str.upper))

The match operation is then performed as::
    
    >>> tftargetdat = yeastract.Match(yeast_feats, _.target, _.feat_name)
...
a bit more thoroughly, to show the use of some other operations. First, we determine which targets where not matched::
    ...
    >>> non_matched = (yeastract.target.Set() - tftargetdat.target.Set()).Elem()
    >>> non_matched.Show()
    Slices: | target
    ---------------------------------------
    Type:   | bytes[11]
...
As you can see, ``|In|`` matches with the last dimensioni of gene_aliases as described before. This means that there are multiple aliases list to be matched, resulting
now in a matrix of matched values. Of course, this is not what we want exactly. We can solve this in two different ways.
...
Another appraoch to get this result is the use of the ``Flat`` operation. This reduces the gene_aliases list to a flat list of entries. We can use it like::
...
We do a quick check if this does not also happen in our matched data::
    ...
    >>> tftargetdat.Unique(_.trans_factor, _.target).target.Count()
    Slices: | target
    ----------------
    Type:   | int32
    Dims:   |
...
This show the use of the filter operation, using the Count operation to determine the size of each group. We could have applied Count also on other slices then 
``feat_name`` with the same result.

Now we select from the yeastract dataset the pairs with these non matched targets::
...
Using the ``Without`` operation, we remove the ``gene_aliases2`` slice, and then stack both datasets to get our final result::
    ...
    >>> tf_feat = Stack(tftargetdat, res).Copy()
...
Of course, this was a lot of extra work for just a few extra matched pairs. However, hopefully it has illustrated several operations that can be performed with Ibidas.
...
Lets calculate a correlation between transcription factor, based on if they adress the same chromosomes::
...
As you can see, it has calculated a square matrix with all correlation coefficients. What if we now want to calculate a correlation between chromosomes instead?::
...
For this we use the transpose operation, which reorders the dimensions of the slices. Of course, from this is hard to identify which columns/rows correspond to which chromosome.
So we like to order on chromosome number. As it is currently a bytes type it would do an alphabetic ordering which is not what we want. So, first we cast it to an integer type::
...
As last step, we like to calculate the chromosome specificness distribution transcription factors. I.e.
how much more often is one chromosome targeted over another?
...
we will normalize our data first::
...
the division operator know on which dimension it should divide the vector that is res.count.Sum("gtrans_factor"). The reason for that is that
it can map them according to dimension identity. Now we can finish::
...
It seems that indeed there is some chromosome specificness in the transcription factors. We plot this again using matplotlib::
