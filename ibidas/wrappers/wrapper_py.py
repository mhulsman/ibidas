...
def rep(data=None, dtype=rtypes.unknown, modifiable=False, unpack=True):#{{{
    ...
    unpacked = False
    if(unpack):
        while(res.getType().__class__ is rtypes.TypeArray):
            unpacked = True
            res = repops_dim.unpack_array(res)

        if(isinstance(res.getType(), rtypes.TypeTuple)):
            unpacked = True
            res = repops_slice.unpack_tuple(res)
    
    if(modifiable or unpacked):
        res = res.copy(modifiable=modifiable)
    return res#}}}
...
class PyRepresentor(wrapper.SourceRepresentor):
    _select_indicator = PySelectIndicator()

    def __init__(self, data, slices):
        all_slices = dict([(slice.id, slice) for slice in slices])
        wrapper.SourceRepresentor.__init__(self, all_slices, slices)
        self._data = data

class VarPyRepresentor(PyRepresentor):
    pass

class ModifiablePyRepresentor(PyRepresentor):
    pass
